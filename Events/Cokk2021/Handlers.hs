...
getParticipantsForUser :: String -> DBHandle User -> DBHandle WaterLog -> IO Response
...
fight :: String -> DBHandle User -> Logger -> IO Response
fight content userDB log =
  Response.processJsonBody content FightRequest.decode $ \req -> do
    ...
    case sourceUserOpt of
      Nothing -> Response.make Unauthorized "Bocs, de rossz a jelszo/felhasznalo"
      Just sourceUser ->
        case targetUserOpt of
          Nothing -> Response.make BadRequest "Bocs de nem letezik az akit meg akarsz ontozni"
          Just targetUser -> do
...
login :: String -> DBHandle User -> DBHandle WaterLog -> Logger -> IO Response
...
register :: String -> DBHandle User -> Settings -> Logger -> IO Response
register content userDB settings log = do
  ...
  then
    Response.processJsonBody content Register.decode $ \registration -> do
      ...
      then Response.make BadRequest "Felhasznalonev nem egyedi."
      else
        if User.eggname user `elem` map User.eggname users
        then Response.make BadRequest "Tojasnev nem egyedi."
        else do
    ...
    Response.make Forbidden "The event is not running anymore!"

water :: String -> DBHandle User -> DBHandle WaterLog -> Logger -> Settings -> IO Response
water content userDB waterDB log settings = do
  ...
  then do
    ...
    Response.processJsonBody content WaterRequest.decode $ \req ->
      ...
      then Response.make BadRequest "Nem ontozheted meg magad! >:|"
      else do
        ...
        then Response.make Forbidden "🖕"
          ...
          maybe
            (DB.insert waterDB wLog >> Response.make OK "Success")
            (Response.make BadRequest)
            result

refreshDashboard :: String -> DBHandle User -> DBHandle WaterLog -> IO Response
...
incSkill :: String -> DBHandle User -> Logger -> Settings -> IO Response
    ...
    Response.processJsonBody content IncSkillRequest.decode $ \req -> do
      ...
      maybe
        ...
        (\user -> do
          ...
          maybe
            (Response.make BadRequest "No such skill exists!")
            (\skill -> do
              let level = skill $ User.skills user
              if level >= 10
              then Response.make Forbidden "You can't increase skill level above 10!"
              else
